...
function findMinMax(numbers) {
  if (!Array.isArray(numbers)) {
    console.error("Помилка: введене значення має бути масивом");
    return null;
  }

  let minArray = Math.min(...numbers);
  let maxArray = Math.max(...numbers);

  let result = `min: ${minArray}, max: ${maxArray}`;
  return result;
...
function calculateHypotenuse(a, b) {
  if (typeof a !== "number" || typeof b !== "number") {
    console.error("Помилка: значення мають бути числами");
    return null;
  }

  let gipotenuzaLength = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
  return gipotenuzaLength;
...
function calculateVolumeCylinder(radius, height) {
  ...
  let volCyl = Math.PI * Math.pow(radius, 2) * height;
  return Math.ceil(volCyl);
...
function getFractionalPart(num) {
  ...
  const roundedFractionalPart = parseFloat(fractionalPart.toFixed(7));
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  if (!Array.isArray(products)) {
    console.error("Помилка: введений об'єкт 'products' має бути масивом");
    return null;
  }

  if (typeof maxPrice !== "number" || isNaN(maxPrice)) {
    console.error("Помилка: введений аргумент 'maxPrice' має бути числом");
    return null;
  }

  const totalPrice = products.reduce((acc, product) => acc + product.price, 0);
  const totalPriceFloat = Math.fround(totalPrice);
  const maxPriceFloat = Math.fround(maxPrice);
  return totalPriceFloat <= maxPriceFloat;
